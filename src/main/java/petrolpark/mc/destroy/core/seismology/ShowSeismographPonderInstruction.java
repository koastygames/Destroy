...
public class ShowSeismographPonderInstruction extends FadeInOutInstruction {
    ...
    public static class Outline {
        public final int x;
        public final int y;
        public final DestroyGuiTexture texture;
        ...
        public Outline(int x, int y, DestroyGuiTexture texture, PonderPalette color, int ttl) {
            this.x = x;
            this.y = y;
    ...
    public class SeismographElement extends AnimatedOverlayElementBase {
        ...
        public void highlightRow(SceneBuilder scene, PonderPalette color, int row, int ticks) {
            scene.addInstruction(s -> outlines.add(new Outline(4, 12 + 6 * row, DestroyGuiTexture.SEISMOGRAPH_HIGHLIGHT_ROW, color, ticks)));
        };

        public void highlightColumn(SceneBuilder scene, PonderPalette color, int column, int ticks) {
            scene.addInstruction(s -> outlines.add(new Outline(12 + 6 * column, 4, DestroyGuiTexture.SEISMOGRAPH_HIGHLIGHT_COLUMMN, color, ticks)));
        };

        public void highlightPlus(SceneBuilder scene, PonderPalette color, int x, int z, int ticks) {
            scene.addInstruction(s -> outlines.add(new Outline(6 + 6 * x, 6 + 6 * z, DestroyGuiTexture.SEISMOGRAPH_HIGHLIGHT_CROSS, color, ticks)));
        };
